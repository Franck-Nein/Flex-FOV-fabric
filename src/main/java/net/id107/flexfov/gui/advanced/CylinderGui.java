...
public class CylinderGui extends AdvancedGui {
	...
	protected void init() {
		...
		DoubleOption FOVX = new DoubleOption("cylinderFovX", 0, 360, 1,
				...
				(gameOptions, doubleOption) -> {return new LiteralText("Horizontal FOV: " + Math.round(Projection.getInstance().getFovX()));});
		...
		DoubleOption FOVY = new DoubleOption("cylinderFovY", 0, 180, 1,
				...
				(gameOptions, doubleOption) -> {return new LiteralText("Vertical FOV: " + Math.round(Projection.getInstance().getFovY()));});
